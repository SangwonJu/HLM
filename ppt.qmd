...
# Module I: 위계적 선형모형의 배경 {background-color="#0AA8AB"}
...
simul %>% 
  ...
  scale_x_continuous(breaks = seq(20, 65, by = 5), limits = c(20,65)) + 
...
-   **Disaggregate?**: POLS $\rightarrow$ independence of obs assumption violated, 개인주의적 오류(individualistic fallacy)
      - 표준오차가 상당히 작아져 귀무가설을 기각할 확률이 매우 높아지는 잘못된 추정결과를 발생시킴
...
::: columns
::: {.column width="50%"}
![](pols.png){fig-align="center"}
:::

::: {.column width="50%"}
![](between.png){fig-align="center"}
:::
:::
...
-   현대 사회에서 인간의 다양한 위계에 nested. 조직 내에서의 군집성으로 인해 서로서로에게 영향을 주게 되고, 결과적으로 개체들 간의 독립성을 가정하는 가우스-마르코프 가정을 충족하는 것이 까다로움

. . . 

-   기존방법은 (1) 개인 간의 완벽한 독립성이 존재하기 어렵고, (2) 자료 자체가 nested되어 있다는 것을 고려하지 못함 $\rightarrow$ Individual 대상 연구에서 (특히, 조직맥락) OLS는 더이상 만능 X, 오류를 범할 수 있게 됨

. . . 

-   **Pooled-OLS**는 집단간 군집성 무시, 심슨의 패러독스, 개인주의적 오류, **Between 모형**은 개개인들의 특성이 고려되지 않는 문제 , 생태학적 오류
...
# Module II: 위계적 선형모형의 개념 {background-color="#0AA8AB"}
...
1.  **개인 수준과 집단 수준의 잔차**를 모델링함을 통해서 결과적으로 OLS대비 동일한 그룹 내의 개인들 간의 부분적인 독립성을 확보함 
...
::: justify
-   **OLS**는 집단에 대한 고려없이 모든 개인들을 동일하게 고려  

$$Y_{i} = \beta_{0} + \beta_{1} X_{i} + \epsilon_{i} \quad \epsilon_i \sim N(0,\sigma^2)$$

-   **HLM**은 집단 내(개인: $i$)과 집단 간(집단: $j$) 모형을 별개로 model specification 한다
    - **Level 1:** 
    ...
    $$\left[\begin{array}{cc} u_{0j} \\ u_{1j} \\ \end{array} \right] \sim N , (\left[\begin{array}{cc} 0 \\ 0 \\ \end{array} \right], \left[\begin{array}{cc} \tau_{00} & \tau_{01} \\ \tau_{10} & \tau_{11} \\ \end{array} \right]) $$
...
## HLM을 통해 해결가능한 연구문제 examples (강상진, 2016) {.smaller}
...
-   기존 회귀분석 방법과 달리 잔차의 독립성에서 자유롭고, 분산을 수준별로 산출가능

# Module III: 위계적 선형모형의 구체화 {background-color="#0AA8AB"}
...
pacman::p_load("tidyverse", "magrittr", # data분석 필수 패키지
               ...
               "lmerTest", "lme4", "merTools", "nlme", # HLM 패키지
               "bruceR", # HLM 결과 정리 표 생성
               "readstata13", # statafile (.dta) 열어주는 패키지 
               "skimr", "psych", # 요약표, 심리학 분석
               "plm") # 패널회귀분석 (within, random)
## 자신이 설정하고 싶은 곳으로 설정
setwd("E:/OneDrive - SNU/(B) 대학원/세미나/HLM"); getwd()

# Read data
data_lv1 <- read.dta13("./HSB1.dta"); data_lv2 <- read.dta13("./HSB2.dta")
```
...
::: columns
::: {.column width="50%"}
![](./dplyrjoin.png){fig-align="center"}

[출처](https://rfriend.tistory.com/625)
:::

::: {.column width="50%"}
![](./joins.png){fig-align="center"}
...
# column 병합
bind_cols(list(data*, data*, ... ), .name_repair = c("unique", "universal"))
```
...
## Data Pivot: `pivot_longer` and `pivot_wider` {.smaller}
...
data_merged %>% 
  ...
  ds4psy::theme_ds4psy() + 
  labs(x = "School id (ordered by meanses)" , y = "수학 성취도", title = "학교별 개인 수학성취도 비교")+
  theme(text=element_text(family='Nanum Gothic'),
...
-   **급간상관계수(ICC)와 신뢰도 계산**이 주 목적으로 모형에 아무런 predictor variable들이 투입되지 않음  
...
$$ ICC(1) = \frac{학교\,간\,분산}{전체\,관찰분산} = \frac{Var(\beta_{0j})}{Var(Y_{ij})}= \frac{\tau}{\sigma^2 + \tau} = \frac{8.614}{39.148 + 8.614} = 0.18035 $$
...
-   SES = 0인 사람들에 대한 수학성취도가 $\gamma_{00}$=12.658으로 추정되는데 평균 SES의 성적
...
\
$$Var(\left[\begin{array}{cc} u_{0j} \\ u_{1j} \\ \end{array} \right])= \left[\begin{array}{cc} \tau_{00} & \tau_{01} \\ \tau_{01} & \tau_{11} \\ \end{array} \right] = \mathrm{T}$$
...
#| output: false
#| code-overflow: wrap
data_merged <- data_merged %>% group_by(id) %>% 
  mutate(meanses = mean(ses),
         ses_groupmc = ses - meanses)
...
3.  Variance Explained (at level 1): explains 6.3% of level-1 variance $$\frac{\hat{\sigma}(Random\,ANOVA)-\hat{\sigma}(SES)}{\hat{\sigma}(Random\,ANOVA)} =\frac{39.148- 36.70019}{39.148} = 0.063$$
...
level.2.data %>%  
  ...
  ggrepel::geom_text_repel(aes(label=id), size = 4)+
...
# Module IV: 중심화와 수준 간 상호작용 {background-color="#0AA8AB"}
...
-   Overall Model Example: (intercept + fixed + random)
    ...
    - fixed effect: $\gamma_{01}(SECTOR_{j}) + \gamma_{10}(SES_{ij}) +\gamma_{11} (SECTOR_j)(SES_{ij})$
...
plot_model(model8, type = 'pred', terms = c('ses_groupmc','sector'), ci.lvl = 0.95) + 
  ...
  ggeasy::easy_center_title() +
  ggeasy::easy_all_text_size(30) +
  labs(x = "SES(Group Mean Centered)" , y = "수학 성취도 (mathach)", title = "Predicted values of mathach",
...
cd "E:/OneDrive - SNU/(B) 대학원/세미나/HLM/3주차 HLM"
use HSB1.dta, clear
